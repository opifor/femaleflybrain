...
def generate():
    ...
    write_feather(ROOT / "male/body-annotations.feather", dict(bodyId=ids, type=types, somaSide=sides,
    ...
    write_feather(ROOT / "banc/banc_888_meta.feather", dict(banc_888_id=list(map(str, large_ids)),
                  root_id=[str(i+1000) for i in large_ids], cell_type=types, side=sides, super_class=classes,
